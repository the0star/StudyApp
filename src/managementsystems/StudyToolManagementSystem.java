...
public abstract class StudyToolManagementSystem {
    ...
    private void study(String studyToolId) {
        ...
        boolean repeatQuestions = templateManager.getWordsReappear(Integer.parseInt(templateId));
        List<Object> timeInfo = templateManager.getTimeInfo(templateId);
        boolean isTimedPerQuiz = (Boolean) timeInfo.get(0) && !(Boolean) timeInfo.get(1);
        boolean isTimedPerQuestion = (Boolean) timeInfo.get(0)&& (Boolean) timeInfo.get(1);
        long timeLimitInMS = 1000 * (Integer) timeInfo.get(2);
        long quizStartTime = System.currentTimeMillis();
        int totalScore = questionsAndAnswers.length;
        int currScore = 0;
        ...
        for (String[] qa : questionsAndAnswers) {
            ...
            boolean correctness = testAndCheckCorrectness(qa, templateType, answerChecker);
            if (repeatQuestions && !correctness) {
                questionsToRepeat.add(qa);
            }
            long questionTimeElapsed = System.currentTimeMillis() - questionStartTime;
            if (isTimedPerQuestion && (questionTimeElapsed >= timeLimitInMS)) {
                regularPresenter.ranOutOfTimeReporter();
            } else {
                currScore += (correctness ? 1 : 0);
        ...
        fcRepeatStudy(questionsToRepeat, answerChecker, templateType);
        displayStudyResult(quizStartTime, isTimedPerQuiz, currScore, totalScore, templateId, timeLimitInMS);
    }

    private boolean testAndCheckCorrectness(String[] qa, TemplateManager.TemplateType templateType,
                                            AnswerChecker answerChecker){
        studyToolDisplayer.displayAQuestion(qa);
        regularPresenter.getAnswerPrompter(templateType);
        String answer = scanner.nextLine();
        return answerChecker.isCorrectAnswer(answer, qa[1]);
    }

    private void displayStudyResult(Long quizStartTime, boolean isTimedPerQuiz, int currScore, int totalScore,
                                    String templateId, Long timeLimitInMS){
        long quizTimeElapsed = System.currentTimeMillis() - quizStartTime;
        if (isTimedPerQuiz && (quizTimeElapsed >= timeLimitInMS)){
            regularPresenter.ranOutOfTimeReporter();
        }
        else if (templateManager.isTemplateScored(templateId)) {
            String score = currScore + "/" + totalScore;
            regularPresenter.studySessionEndedReporter(score);
        }
    }

    private void fcRepeatStudy(Queue<String[]> questionsToRepeat, AnswerChecker answerChecker,
