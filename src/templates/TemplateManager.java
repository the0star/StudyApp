...
public class TemplateManager implements Serializable {
    ...
    /**
     * Get whether or not the words in this template should be retested if the provided answer is not correct
     */

    public boolean getWordsReappear(int templateId){
        if (getTemplateType(templateId).equals(TemplateType.FC)){
            return ((FlashCardTemplate)this.idMapper.get(templateId)).getWordsReappear();
        }
        return false;
    }
